...
- **Lock Context Flexibility**: When a `SlotLock` is active, the system prompt injects a `LOCKED SLOT` context. This context permits the user to release and reselect a different slot — the LLM is explicitly instructed to call `release_slot` first before helping with a new time, preventing the user from getting trapped on a locked slot.
- **Date Reference Injection**: The 30-day calendar mapping is injected as a newline-separated list (not a comma blob) to prevent LLM date/weekday hallucinations. Rule #7 dynamically references the provider's actual working days from `ProviderSettings` rather than hardcoding Monday–Friday.
