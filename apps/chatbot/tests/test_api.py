...
class TestStartSessionView:
    def test_start_session_anonymous_no_auth(self, api_client):
        """POST /api/chat/sessions/ must work with no auth token."""
        response = api_client.post("/api/chat/sessions/")
        assert response.status_code == 201
        assert "session_key" in response.data
        assert ConversationSession.objects.count() == 1

    def test_start_session_returns_uuid(self, api_client):
        response = api_client.post("/api/chat/sessions/")
        assert response.status_code == 201
    ...
    def test_start_session_user_email_initially_empty(self, api_client):
        response = api_client.post("/api/chat/sessions/")
        assert response.status_code == 201
        assert response.data["user_email"] in [None, ""]
...
class TestSendMessageView:
    ...
    def test_send_message_no_auth_required(self, api_client, session):
        """POST /api/chat/message/ must work with no auth token."""
        with patch("apps.chatbot.views.run_agentic_loop", return_value="Hello! How can I help?"):
            payload = {
                "session_key": str(session.session_key),
                "message": "Hi",
            }
            response = api_client.post("/api/chat/message/", payload, format="json")
        assert response.status_code == 200
        assert response.data["response"] == "Hello! How can I help?"
...
class TestSessionHistoryView:
    ...
    def session_with_messages(self):
        session = ConversationSession.objects.create(user_email="hist@example.com")
    ...
    def test_history_no_auth_required(self, api_client, session_with_messages):
        response = api_client.get(
            f"/api/chat/sessions/{session_with_messages.session_key}/messages/"
        )
        assert response.status_code == 200
    ...
    def test_history_shows_user_email(self, api_client, session_with_messages):
        ...
        assert response.data["user_email"] == "hist@example.com"

    def test_history_unknown_session_returns_404(self, api_client):
        response = api_client.get(f"/api/chat/sessions/{uuid.uuid4()}/messages/")
        assert response.status_code == 404


@pytest.mark.django_db
class TestDeleteSessionView:
    def test_delete_session_no_auth_required(self, api_client):
        session = ConversationSession.objects.create()
        response = api_client.delete(f"/api/chat/sessions/{session.session_key}/")
    ...
    def test_delete_unknown_session_returns_404(self, api_client):
        response = api_client.delete(f"/api/chat/sessions/{uuid.uuid4()}/")
        assert response.status_code == 404
